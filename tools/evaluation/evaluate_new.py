...
from tree_learn.util import (get_root_logger, make_labels_consecutive, get_config,
                             ...
                             evaluate_instance_segmentation, propagate_preds, load_data, voxelize, get_instance_assessment)
...
def evaluate(config, config_path=None):
    ...
    #shift and rescale pred forest to benchmark forest
    config.shift_and_rescale = True
    if config.shift_and_rescale:
        benchmark_forest[:, :3] = benchmark_forest[:, :3] - np.min(benchmark_forest[:, :3], axis=0)
        pred_forest[:, :3] = pred_forest[:, :3] - np.min(pred_forest[:, :3], axis=0)
        pred_forest[:, :3] = pred_forest[:, :3] / np.max(pred_forest[:, :3], axis=0) * np.max(benchmark_forest[:, :3], axis=0)
    ...
    for voxel_size in voxel_sizes:
        ...
        benchmark_forest = voxelize(benchmark_forest, voxel_size)
        pred_forest = voxelize(pred_forest, voxel_size)
        ...
        instances_to_evaluate = np.unique(benchmark_forest_instance_labels)
        ...
        torch.save(results_dict, os.path.join(config.base_dir, f'instance_evaluation_{voxel_size}.pt'))
        all_results[voxel_size] = results_dict

    metrics = ['prec', 'rec', 'f1', 'iou']
    results_to_plot = dict((el,[]) for el in metrics)
    for voxel_size, results in all_results.items():
        for metric in metrics:
            m = results['segmentation_results']['no_partition'][metric].mean(0)
            results_to_plot[metric].append(m)

    #save to csv file
    df = pd.DataFrame(results_to_plot)
    df['voxel_size'] = voxel_sizes
    df.to_csv(os.path.join(config.base_dir, 'results.csv'))


    # for metric in metrics:
    #     print(metric)
    #     print(results_to_plot[metric])
    #     plt.plot(voxel_sizes, results_to_plot[metric], label=metric)
    #     plt.show()
